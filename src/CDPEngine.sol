...
contract Vat {
    ...
    function addAuthorization(address usr) external emitLog onlyOwners { require(DSRisActive == 1, "Vat/not-DSRisActive"); authorizedAccounts[usr] = 1; }
    function removeAuthorization(address usr) external emitLog onlyOwners { require(DSRisActive == 1, "Vat/not-DSRisActive"); authorizedAccounts[usr] = 0; }
    modifier onlyOwners {
        require(authorizedAccounts[msg.sender] == 1, "Vat/not-onlyOwnersorized");
        _;
    }
    ...
    function file(bytes32 what, uint data) external emitLog onlyOwners {
        require(DSRisActive == 1, "Vat/not-DSRisActive");
    ...
    // --- Fungibility ---
    function slip(bytes32 ilk, address usr, int256 wad) external emitLog onlyOwners {
        gem[ilk][usr] = add(gem[ilk][usr], wad);
    }
    function flux(bytes32 ilk, address src, address dst, uint256 wad) external emitLog {
    ...
    // --- Rates ---
    function fold(bytes32 i, address u, int rate) external emitLog onlyOwners {
        require(DSRisActive == 1, "Vat/not-DSRisActive");
        Ilk storage ilk = ilks[i];
